...
export default function YouTubeSEOChecklist() {
  ...
  // Extraí ID do vídeo em qualquer formato de URL do YouTube
  const extractVideoId = (url: string) => {
    const regex = /(?:https?:\/\/)?(?:www\.)?(?:m\.)?(?:youtube\.com|youtu\.be)(?:\/(?:watch\?v=|embed\/|shorts\/)?)([a-zA-Z0-9_-]{11})/;
    const match = url.match(regex);
    return match ? match[1] : null;
  };

  // Mapeia categoryId para nome legível
  ...
  const checkField = (field: any) => (field ? "✅" : "❌");
            ...
            <table className="w-full border-collapse border border-gray-300 text-sm">
              ...
              <tbody>
                <tr>
                  ...
                  <td className="border border-gray-300 p-2 text-center">{checkField(videoData.title)}</td>
                ...
                <tr>
                  ...
                  <td className="border border-gray-300 p-2 text-center">⭐⭐⭐⭐⭐</td>
                  <td className="border border-gray-300 p-2 text-center">{checkField(videoData.description)}</td>
                  <td className="border border-gray-300 p-2">Importante para engajamento</td>
                </tr>
                <tr>
                  <td className="border border-gray-300 p-2">Tags</td>
                  <td className="border border-gray-300 p-2 text-center">⭐⭐⭐</td>
                  <td className="border border-gray-300 p-2 text-center">{checkField(videoData.tags)}</td>
                ...
                <tr>
                  ...
                  <td className="border border-gray-300 p-2 text-center">⭐⭐</td>
                  <td className="border border-gray-300 p-2 text-center">{checkField(videoData.categoryId)}</td>
                  <td className="border border-gray-300 p-2">{categories[videoData.categoryId] || "Categoria não definida"}</td>
                </tr>
                <tr>
                  <td className="border border-gray-300 p-2">Idioma do vídeo</td>
                  <td className="border border-gray-300 p-2 text-center">⭐⭐</td>
                  <td className="border border-gray-300 p-2 text-center">{checkField(videoData.defaultAudioLanguage)}</td>
